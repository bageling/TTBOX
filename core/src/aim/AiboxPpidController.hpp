...
public:
 ...
 double update(double e){
  ...
  const double output_scale = smooth_ > 0.0 ? bandwidth_ - smooth_ : bandwidth_ - 9900.0;
  double kp=soft(kp_*e,output_scale), ki=soft(ki_raw,bandwidth_ - 1000.0), kd=soft(kd_*diff,output_scale);
...
private:
 ...
 double soft(double v,double output_scale)const{double r=v/bandwidth_,s=r*r;return r*(1.0+(4.0/15.0)*s)/(1.0+(3.0/5.0)*s)*output_scale;}
 double kp_=25,kd_=25,predict_=0,kp_rate_=0.3,smooth_=9900,bandwidth_=10000.0;double kp_gain_=0,integral_gain_=0,u_=0,last_error_=0;double vx_=0,vp_=0,ix_=0,ip_=0;
};}
