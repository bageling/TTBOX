...
int main(int argc,char** argv){
    std::string model; std::string device="/dev/video0"; std::string fifo="/run/ttbox-aim.fifo"; int workers=1; int seconds=10; bool trace=false; bool fifo_mode=false; bool simulate=false; bool aibox_gadget=false;
    for(int i=1;i<argc;++i){std::string a=argv[i]; auto next=[&](){return i+1<argc?std::string(argv[++i]):std::string();};
        if(a=="--model")model=next(); else if(a=="--device")device=next(); else if(a=="--workers")workers=std::atoi(next().c_str()); else if(a=="--seconds")seconds=std::atoi(next().c_str()); else if(a=="--trace")trace=true; else if(a=="--fifo")fifo_mode=true; else if(a=="--simulate-hotkey")simulate=true; else if(a=="--aibox-gadget")aibox_gadget=true;}
    ...
    std::printf("hardware_runner_main: running seconds=%d workers=%d output=%s\n", seconds, workers, aibox_gadget ? "aibox-hidg1" : (fifo_mode ? "fifo" : (trace ? "trace" : "null")));
