...
if [ -z "$MODEL" ] && [ -f /opt/ttbox/models/current/model.rknn ]; then
  MODEL=/opt/ttbox/models/current/model.rknn
fi
if [ -z "$MODEL" ]; then
...
stdbuf -oL "$BIN" --model "$MODEL" --adapter --workers "$WORKERS" --cores "$CORES" \
  ...
  --report-every 5 --mouse-fifo /run/ttbox-aim.fifo \
  2>&1 | tee "$LOG"
