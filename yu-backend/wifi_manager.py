...
DEFAULT_PASSWORD = os.environ.get("AIASSISTANCE_WIFI_DEFAULT_PASSWORD", "12345678")
_default_ssids_raw = os.environ.get("AIASSISTANCE_WIFI_DEFAULT_SSIDS")
if _default_ssids_raw is None:
    _default_ssids_raw = f"{os.environ.get('AIASSISTANCE_WIFI_DEFAULT_SSID', 'YUAI')} XCSH XHAI"
DEFAULT_SSIDS = [item.strip() for item in re.split(r"[\s,]+", _default_ssids_raw) if item.strip()] or ["YUAI", "XCSH", "XHAI"]
DEFAULT_SSID = DEFAULT_SSIDS[0]
DEFAULT_CONNECTION_PREFIX = os.environ.get("AIASSISTANCE_WIFI_DEFAULT_CONNECTION_PREFIX", "aiassistance-default-")
USER_CONNECTION_PREFIX = os.environ.get("AIASSISTANCE_WIFI_USER_CONNECTION_PREFIX", "aiassistance-wifi-")
AP_CONNECTION = os.environ.get("AIASSISTANCE_WIFI_AP_CONNECTION", "aiassistance-ap-hotspot")
WIFI_BOOTSTRAP_SERVICE = os.environ.get("AIASSISTANCE_WIFI_BOOTSTRAP_SERVICE", "aiassistance-wifi-bootstrap.service")
...
def _web_port() -> int:
    try:
        port = int(os.environ.get("AIASSISTANCE_PORT", "") or DEFAULT_WEB_PORT)
...
def _default_connection_name(ssid: str) -> str:
    legacy = os.environ.get("AIASSISTANCE_WIFI_DEFAULT_CONNECTION", "").strip()
...
def _ap_status(iface: str = "", connected: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "mdns_url": _web_url("aiassistance.local"),
