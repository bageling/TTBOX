...
MODES: dict[str, VideoMode] = {
    ...
    # 高刷 1080p（compat=CVT-RB 风格，余量更足；标准版用 1080p60 相同 porch/sync）
    '1080p144': VideoMode('1080p144', 1920, 1080, 144, 348941, 48, 32, 80, 3, 5, 77, None, True, False),
    '1080p240': VideoMode('1080p240', 1920, 1080, 240, 594000, 88, 44, 148, 4, 5, 36, None),
    '1080p240compat': VideoMode('1080p240compat', 1920, 1080, 240, 581568, 48, 32, 80, 3, 5, 77, None, True, False),
    # 1440p 高刷（CVT-RB 风格）
    '1440p60': VideoMode('1440p60', 2560, 1440, 60, 248875, 48, 32, 80, 3, 5, 77, None, True, False),
    '1440p120': VideoMode('1440p120', 2560, 1440, 120, 497750, 48, 32, 80, 3, 5, 77, None, True, False),
    '1440p144': VideoMode('1440p144', 2560, 1440, 144, 586345, 48, 32, 80, 3, 5, 49, None, True, False),
    '2160p60': VideoMode('2160p60', 3840, 2160, 60, 594000, 88, 44, 428, 4, 5, 81, 97),
}
ALIASES = {
    'fhd60': '1080p60',
    ...
    '1k144': '1080p144',
    'fhd240': '1080p240',
    'fhd240compat': '1080p240compat',
    '1k240': '1080p240',
    '1k240compat': '1080p240compat',
    '2k60': '1440p60',
    'qhd60': '1440p60',
    '2k120': '1440p120',
    'qhd120': '1440p120',
...
def pack_hdmi_forum_vsdb(max_tmds_mhz: int) -> bytes:
    ...
    # byte6: bit7 = SCDC_Present（0x80），无 SCDC 声明时 340MHz+ 源不启用
    #        TMDS Bit Clock Ratio ×2 / scrambling → 高刷锁不住（对照 239 已验证 EDID）
    payload = bytes([0xD8, 0x5D, 0xC4, 0x01, max_tmds_code, 0x80, 0x00])
    return pack_data_block(0x03, payload)
...
def build_base_block(name: str, vendor: str, product_id: int, serial: int, modes: list[VideoMode]) -> bytearray:
    ...
    block[18] = 1
    block[19] = 4
    block[20] = 0xA2  # 数字输入 DFP + 8bpc，对齐 239 已验证 EDID
    block[21] = 0x00
    block[22] = 0x00
    block[23] = 0x78
