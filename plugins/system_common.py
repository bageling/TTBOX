...
class UpgradeService(CommandAdapter):
    # 占位适配器：离线升级能力（scripts/update_engine.py 及其 /api/update/* 端点）
    # 已按产品决策整体移除，这里保留结构只为让插件聚合不缺口，不再指向任何实现。
    def status(self): return {"state": "unavailable", "source": "upgrade-adapter-removed"}
