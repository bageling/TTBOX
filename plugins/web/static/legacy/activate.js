...
async function waitForFullRecovery() {
  ...
  // 离线更新能力（含全部 /api/update/* 端点）已按产品决策下线，取 /api/update/status
  // 会得到 404/405。这种情况下不再空转满 3 分钟，直接优雅降级收尾。
  const UPDATE_STATUS_REMOVED = new Set([404, 405, 501]);
  while (Date.now() < deadline) {
    await new Promise((resolve) => window.setTimeout(resolve, 2000));
    let payload = null;
    try {
      payload = await api("/api/update/status");
    } catch (error) {
      if (error && UPDATE_STATUS_REMOVED.has(error.status)) {
        recoveryLocked = false;
        setStatus("本地全量恢复已停用：更新状态接口不再提供。", true);
    ...
    if (payload.status === "failed" && payload.recovery) {
      recoveryLocked = false;
    ...
    if (payload.status === "success" && payload.stage === "recovery_complete") {
      setStatus("本地全量恢复完成，正在刷新激活页面。");
      window.setTimeout(() => window.location.reload(), 800);
      return;
    }
    if (payload.recovery && payload.message) {
