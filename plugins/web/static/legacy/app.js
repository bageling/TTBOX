...
const NUMERIC_RANGE_LIMITS = {
  ...
  // ---- 持续提前量（对标 yu controller.continuous_lead_*）----
  controller_continuous_lead_enter_distance: [0, 2000],
  controller_continuous_lead_scale: [0, 2],
  controller_continuous_lead_fade_in_ms: [0, 3000],
  controller_continuous_lead_fade_out_ms: [0, 3000],
...
const CONTROLLER_DEFAULTS = {
  ...
  // 持续提前量：默认"关"，与 Core 侧安全默认一致（不开就不动输出链）。
  // 另注：fade_in_ms / fade_out_ms / near_disable_ratio 为 Core 侧**预留字段**
  // （ContinuousLead.hpp 的 apply() 当前未消费），面板上标为"预留"。
...
// ---------------------------------------------------------------------------
// 品牌（ui_brand）解析 —— 对标 yu 的换皮机制（app.py:362-427）
// ---------------------------------------------------------------------------
// 数据流：签名卡 → Core → /api/state.ui.ui_brand（或 /api/license.ui_brand）→ 本文件。
//
// 分工：
//   · 服务端 plugins/web/config/ui_brands.json 是**权威注册表**，
//     并把解析好的展示字段一并挂在 payload.ui 里下发（brand_title/brand_mark/
//     brand_eyebrow/app_title/default_theme/allow_theme_switch/...）。
//   · 本镜像表用途单一：在**只拿到 token**（SSR 的 <html data-ui-brand>，
//     首帧还没 fetch 到 JSON）时也能立刻渲染正确文案，避免品牌串台/闪字。
//
// 闭集语义与服务端 _normalize_ui_brand 完全一致：token 命中不了就回退原厂。
// 两侧各自独立收窄（服务端收窄一次、客户端再收窄一次），
// 任何一侧被篡改都不会让未知 token 渲染成空白面板。
const UI_BRAND_REGISTRY = {
  ttbox: {
    title: "TTBOX 控制台",
    appTitle: "TTBOX 控制台",
    ...
    showCloudEncryptedModels: true,
  },
};

function normalizeUiBrand(value) {
  const token = String(value == null ? "" : value).trim().toLowerCase();
  return Object.prototype.hasOwnProperty.call(UI_BRAND_REGISTRY, token)
    ? token
    : UI_BRAND_TTBOX;
}

// 从任意 API 载荷取品牌 token。兼容四种形态：
//   /api/state 整包（ui_brand 顶层 / ui.ui_brand）
//   /api/license 整包（ui_brand 顶层 / ui.ui_brand）
//   renderLicensePanel 的裁剪包（{license, core, ...}）
//   SSR 的 <html data-ui-brand>（兜底）
...
// 服务端已解析好的展示字段（权威）。返回 null 表示"该字段服务端没给"，
// 区别于空字符串（空字符串是服务端明确给了空值）。
function brandWireFields(payload) {
  ...
  return {
    title: pick(src.brand_title),
    appTitle: pick(src.app_title),
    eyebrow: pick(src.brand_eyebrow),
    mark: pick(src.brand_mark),
    allowThemeSwitch: typeof src.allow_theme_switch === "boolean" ? src.allow_theme_switch : null,
    defaultTheme: (src.default_theme === "light" || src.default_theme === "dark") ? src.default_theme : null,
    defaultLocalName: pick(src.default_local_name),
    defaultHotspotSsid: pick(src.default_hotspot_ssid),
    fallbackResetText: pick(src.fallback_reset_text),
  };
}

function brandConfig(brand, payload) {
  const id = normalizeUiBrand(brand);
  const mirror = UI_BRAND_REGISTRY[id] || UI_BRAND_REGISTRY[UI_BRAND_TTBOX];
  const config = {
    uiBrand: id,
    title: mirror.title,
    appTitle: mirror.appTitle || mirror.title,
    eyebrow: mirror.eyebrow,
    mark: mirror.mark,
    allowThemeSwitch: mirror.allowThemeSwitch,
    defaultTheme: mirror.defaultTheme,
    defaultLocalName: mirror.defaultLocalName,
    defaultHotspotSsid: mirror.defaultHotspotSsid,
    fallbackResetText: mirror.fallbackResetText,
    // 原厂专属能力（cloud_encrypted 模型后端）。旧实现是
    // state.uiBrand === UI_BRAND_TTBOX 的硬相等判断；数据化后语义不变：
    // 注册表未显式声明 ⇒ 仅原厂品牌为 true。
    showCloudEncryptedModels: typeof mirror.showCloudEncryptedModels === "boolean"
      ? mirror.showCloudEncryptedModels
      : (id === UI_BRAND_TTBOX),
  };
  // 服务端字段覆盖镜像（渠道卡换文案/主题无需重发前端包）
  const wire = brandWireFields(payload);
  if (wire.title) config.title = wire.title;
  if (wire.appTitle) config.appTitle = wire.appTitle;
  if (wire.eyebrow) config.eyebrow = wire.eyebrow;
  if (wire.mark) config.mark = wire.mark;
  if (wire.allowThemeSwitch !== null) config.allowThemeSwitch = wire.allowThemeSwitch;
  if (wire.defaultTheme !== null) config.defaultTheme = wire.defaultTheme;
  if (wire.defaultLocalName) config.defaultLocalName = wire.defaultLocalName;
  if (wire.defaultHotspotSsid) config.defaultHotspotSsid = wire.defaultHotspotSsid;
  if (wire.fallbackResetText) config.fallbackResetText = wire.fallbackResetText;
  return config;
}

function currentBrandConfig() {
  return brandConfig(state.uiBrand, state.data);
}
...
function applyBrand(payload) {
  const config = brandConfig(brandFromPayload(payload), payload);
  state.uiBrand = config.uiBrand;
  document.documentElement.dataset.uiBrand = config.uiBrand;
  // 品牌类：先清旧的再加新的。
  // 原实现固定 add("ui-brand-ttbox") 且从不移除，换品牌后旧类残留
  // ⇒ 两套皮肤 CSS 同时命中，面板会出现混合样式。
  Array.from(document.body.classList)
    .filter((name) => name.startsWith("ui-brand-"))
    .forEach((name) => document.body.classList.remove(name));
  document.body.classList.add(`ui-brand-${config.uiBrand}`);
  ...
  document.title = config.appTitle || config.title;
  ...
  // 主题优先级：自定义视觉主题色 > 用户已存偏好 > 品牌默认主题。
  // 原厂 ttbox（defaultTheme=dark）行为与改动前逐字节一致；
  // 渠道品牌可声明 defaultTheme=light 且 allowThemeSwitch=false
  // （对标 yu 的 xh：浅色且锁死不换）。
  const visualThemeColor = document.documentElement.dataset.visualThemeColor || "system";
  const theme = ["light", "dark"].includes(visualThemeColor)
    ? visualThemeColor
    : (hasStoredTheme() ? storedTheme() : config.defaultTheme);
  applyTheme(theme);
}
...
// ---------------------------------------------------------------------------
// T1.15：模型输入通路 = "换了模型后到底走了哪条快路径"
// 三个 pass_mode 串是 core 的**稳定契约**（rknn/InputQuant.hpp::input_pass_mode_name）；
// 前端只做"串 → 中文标签"的展示翻译 + unknown 兜底，**不重算谓词**
// （谓词与"为什么回落"的文案唯一来源在 core，见 rknn/InputPathSummary.hpp）。
// ---------------------------------------------------------------------------
const MODEL_INPUT_PASS_MODE_LABELS = {
  xor_shift128: "XOR 快路径（CPU拷贝+XOR）",
  uint8_native: "UINT8 原生直传",
  compatible: "兼容 I/O（每帧内部量化）",
};

function formatModelInputPassMode(mode) {
  if (!mode || mode === "unknown") {
    return "未运行";
  }
  return MODEL_INPUT_PASS_MODE_LABELS[mode] || `未知通路（${mode}）`;
}

// 快路径 = xor_shift128 或 uint8_native。compatible 不是故障（FP16 模型也照跑），
// 只是每帧 set_input 代价高一个量级，故文案是"慢"而非"错"。
function isModelInputFastPath(mode) {
  return mode === "xor_shift128" || mode === "uint8_native";
}

function formatModelInputScale(value) {
  const n = Number(value);
  if (!Number.isFinite(n) || n === 0) {
    return "0";
  }
  return n < 0.01 ? n.toExponential(3) : n.toFixed(4);
}

// 输入张量描述："int8/nhwc/affine_asymmetric · zp=-128 · scale=3.922e-3 · 640×640"
function formatModelInputTensor(modelInput) {
  const mi = modelInput || {};
  const shape = [mi.tensor_type, mi.tensor_format, mi.quant_type]
    .filter((part) => part && part !== "unknown")
    .join("/");
  const dims = (Number(mi.model_width) > 0 && Number(mi.model_height) > 0)
    ? `${mi.model_width}×${mi.model_height}`
    : "";
  // ★ 完全没有可报内容（Core 未运行 / 旧固件不带该字段）⇒ 明确给 "-"。
  //   绝不能拼出 "zp=0 · scale=0" —— 那看起来像"实测到 zp=0、scale=0"，
  //   是把"没有数据"伪装成"有数据"，正是本卡要消灭的那类谎报。
  if (!shape && !dims) {
    return "-";
  }
  const parts = [
    shape,
    `zp=${Number(mi.zero_point) || 0}`,
    `scale=${formatModelInputScale(mi.scale)}`,
    dims,
  ].filter(Boolean);
  return parts.join(" · ");
}

// worker 间偏斜：3 个 worker 只有 2 个吃到快路径时必须显式说出，否则平均值会掩盖问题。
function formatModelInputWorkerSkew(modelInput) {
  const mi = modelInput || {};
  const total = Number(mi.workers_total) || 0;
  const fast = Number(mi.workers_fast_path) || 0;
  if (total <= 1 || fast >= total) {
    return "";
  }
  return `（快路径 ${fast}/${total} worker）`;
}
...
function shouldShowCloudEncryptedModelsInLibrary() {
  return !!currentBrandConfig().showCloudEncryptedModels;
}
...
function populateForm(config) {
  ...
  setValue("controller_continuous_lead_near_disable_ratio", controller.continuous_lead_near_disable_ratio ?? CONTROLLER_DEFAULTS.continuous_lead_near_disable_ratio);
  ...
  return {
    ...
    ai: {
      controller: {
        ...
        continuous_lead_near_disable_ratio: getNumber("controller_continuous_lead_near_disable_ratio", CONTROLLER_DEFAULTS.continuous_lead_near_disable_ratio),
...
function renderRuntime(payload) {
  ...
  if (runtimeSummary) {
    ...
    // T1.15：输入通路诊断（未运行时一律显示"未运行"，绝不把上次运行的结论当现状）
    const modelInput = runtime.model_input || {};
    const modelInputPassMode = isRunning ? formatModelInputPassMode(modelInput.pass_mode) : "未运行";
    const modelInputFast = isRunning && isModelInputFastPath(modelInput.pass_mode);
    const modelInputSkew = isRunning ? formatModelInputWorkerSkew(modelInput) : "";
    const modelInputTensor = isRunning ? formatModelInputTensor(modelInput) : "-";
    const modelInputNote = isRunning ? (modelInput.note || "") : "";
    ...
    runtimeSummary.innerHTML = `
      ...
      <div class="runtime-stat">
        <span>输入通路</span>
        <strong class="${modelInputFast ? "stat-ok" : "stat-warn"}">${escapeHtml(modelInputPassMode)}${escapeHtml(modelInputSkew)}</strong>
        <small>${escapeHtml(modelInputTensor)}</small>
        ${modelInputNote ? `<small class="stat-note">${escapeHtml(modelInputNote)}</small>` : ""}
      </div>
    `;
  }
