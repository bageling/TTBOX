...
| **`default.json` 的 `cloud.app_secret` 世界可读**（遗留项，本轮未收紧） | `ls -l /opt/ttbox/config/default.json` 若为 **0644** | **暂不收紧（保持出厂 0644）**：实测 0640 root:ttbox 会与「以 **root** 重写该文件」冲突——root 原子写把文件重建为 `root:root` ⇒ `User=ttbox` 的 web EACCES ⇒ 云端凭据丢失（激活 **502「云端凭据未配置」**）。根治需写侧保留属组（或 ACL），另行排期 |
