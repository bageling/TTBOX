...
| upgrade | process | ✅ | bin/ttbox-upgrade → plugin_entry.py | 占位升级适配器（离线升级能力已移除，仅返回 state=unavailable） |
...
**静态**：style.css / app.js / apiClient.js / motion_training.js / activate.js
...
- UpgradeService: 占位适配器（status 返回 state=unavailable；离线升级能力已移除，不再指向 update_engine）
