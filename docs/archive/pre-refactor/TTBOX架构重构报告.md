...
- CTest：**14/14 通过**（**第一阶段盘点时点**，命令 `ctest --test-dir core/build -C Release --output-on-failure`）。★ 历史值，**并非现况**；现况以最新测试报告 / `ctest -N` 为准（计数锚定，见 `ttbox-vs-yu-program/m1-acceptance-checklist.md` §0 规则 5）。
