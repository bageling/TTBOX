...
| 拉枪曲线 pull_curve | controller_pull_curve_* | GET/PUT /api/config | ✅ | PullCurve.hpp + **AimThread 实际消费**（`core/src/aim/AimThread.cpp:260-261`，输出链 personal_gain 之后、deadzone 之前） | 浏览器 strength 0.9→1.11→恢复 | **REAL**（2026-09-17 复核：先前"输出消费 PLANNED"已过时，实际在调用） |
| 持续提前量 continuous_lead | controller_continuous_lead_* | GET/PUT /api/config | ✅ | **已接线（2026-09-17）**：`MouseTypes.hpp` 加 `mouse.continuous_lead` 成员 → `RuntimeProfile.cpp` 读写 → `AimThread.cpp` 在 pull_curve 之后、recoil 之前注入 `scaled_x`（喂 PID 输出 `aibox_x/y`，**不喂 scaled_***，使 enter 阈值与用户灵敏度解耦）。<br>★ **6 字段中仅 3 个生效**：`enabled` / `enter_distance` / `scale`；<br>`fade_in_ms` / `fade_out_ms` / `near_disable_ratio` 为 Core 侧标"保留字段"、`ContinuousLead::apply()` 尚未消费的**预留项** ⇒ 面板标「预留·调节无效」并设 `readonly`（不糊弄用户）。<br>默认 `enabled=false` ⇒ 未开启时输出链与加入前完全一致 | 面板「04 持续提前量」开关 + 触发累计距离 + 提前量比例 | **REAL**（有效 3 字段）/ **RESERVED**（另 3 字段） |
...
| **模型输入通路诊断**（新增 2026-09-17，T1.15） | 运行数据 → 「输入通路」卡 | GET /api/state（`state.model_input`）+ GET /api/core/status（`model_*`，两面同源） | ✅ | `WorkerStats.input_path` 快照（`WorkerPool.cpp` start 内）→ 纯函数 `summarize_input_path` 聚合（`rknn/InputPathSummary.hpp`）→ `PipelineMetrics.model_*`（18 字段）→ `IpcServer.cpp` 序列化 | 面板显示「通路名（绿=快/琥珀=慢）+ 输入张量 + 回落原因小字 + worker 偏斜」；未运行时显示「未运行」，**绝不**把无数据谎报成 compatible | **REAL**（链路 + 三层单测：纯函数矩阵交叉锁 / IPC 线格式契约 / Web 投影；均已做「拆掉即变红」反向验证）/ **VERIFY**（板端面板实测） |
...
## 10 系统更新（update-page）—— 已移除

产品决策：U 盘离线更新功能整体下线。`scripts/update_engine.py`、
全部 `/api/update/*` 端点、静态脚本 `static/update.js` 以及 index.html 中的
`update-page` 页面与导航项均已删除。下方表格为历史留档，勿再据此实现。

| 功能 | UI 控件 | API | Gateway | Core/OS | 真机 | 状态 |
|------|---------|-----|---------|---------|------|------|
| 更新状态 | 更新页状态 | GET /api/update/status | ✅ | Update Engine | ✅ | ⛔ REMOVED |
| 检查 OTA | 检查按钮 | POST /api/update/check | ✅ | Update Engine | ✅ | ⛔ REMOVED |
| USB 扫描 | USB 扫描按钮 | POST /api/update/scan-otg | ✅ | Update Engine | ✅ | ⛔ REMOVED |
| 开始更新 | 开始按钮 | POST /api/update/start | ✅ | Update Engine | ✅ | ⛔ REMOVED |
| 回滚 | 回滚按钮 | POST /api/update/rollback | ✅ | Update Engine | ✅ | ⛔ REMOVED |
| 取消 | 取消按钮 | POST /api/update/cancel | ✅ | Update Engine | ✅ | ⛔ REMOVED |
| 更新日志 | 日志视图 | GET /api/update/log | ✅ | Update Engine | ✅ | ⛔ REMOVED |
...
| 位置 | 内容 | 处理 |
|------|------|------|
| app.js | 历史品牌分支（ttbox 单品牌化） | ✅ 已收口（2026-09-17）：`normalizeUiBrand/brandFromPayload/brandConfig` 此前无视入参、恒返回 ttbox；现为闭集注册表 + 服务端字段覆盖。见 `plugins/web/config/ui_brands.json`、`plugins/web/tests/test_web_brand.py`（35 用例） |
| plugins/web/bin/ttbox-web.py | /api/hailo/*（无硬件） | 标记 PLANNED 或保留 |
