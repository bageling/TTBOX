...
TTBOX Web UI
    ↓ HTTP API
TTBOX Update Engine (/var/run/ttbox/update.sock)  ← ⛔ 已整体移除，见「Update（更新）—— 已移除」
    ↓ Unix Socket IPC
TTBOX Update Engine (C++/Python)                  ← ⛔ 同上
```
...
> 产品决策：U 盘/离线更新功能（`scripts/update_engine.py` 与全部 `/api/update/*`
> 端点）已整体删除，不再对外提供。原 FIXME 如下，仅作历史留档。

| 方法 | 路径 | 描述 | 状态 |
|------|------|------|------|
| ~~GET~~ | ~~/api/update/status~~ | 获取更新状态 | ⛔ REMOVED |
| ~~POST~~ | ~~/api/update/check~~ | 检查 OTA 更新 | ⛔ REMOVED |
| ~~POST~~ | ~~/api/update/scan-otg~~ | 扫描 USB 更新 | ⛔ REMOVED |
| ~~POST~~ | ~~/api/update/start~~ | 开始更新指定版本 | ⛔ REMOVED |
| ~~POST~~ | ~~/api/update/rollback~~ | 回滚到上一版本 | ⛔ REMOVED |
| ~~POST~~ | ~~/api/update/cancel~~ | 取消更新 | ⛔ REMOVED |
| ~~GET~~ | ~~/api/update/log~~ | 获取更新日志 | ⛔ REMOVED |
...
- ~~Unix Domain Socket：`/var/run/ttbox/update.sock`（Update Engine）~~ ⛔ 随离线更新功能一并移除
- 请求格式：JSON 文本行，以 `\n` 结尾
- 响应格式：JSON 文本行，以 `\n` 结尾
...
#### Update Engine IPC — ⛔ 已移除（原为新增计划）

离线更新能力（`scripts/update_engine.py` 及其 socket）已按产品决策删除，
下列命令不再规划实现：

| 命令 | 描述 | 状态 |
|------|------|------|
| ~~GET_STATUS~~ | 获取更新状态 | ⛔ REMOVED |
| ~~CHECK_UPDATE~~ | 检查 OTA 更新 | ⛔ REMOVED |
| ~~SCAN_OTG~~ | 扫描 USB 更新 | ⛔ REMOVED |
| ~~START_UPDATE~~ | 开始更新 | ⛔ REMOVED |
